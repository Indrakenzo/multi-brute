...
# --- KONFIGURASI GLOBAL ---
# Dikosongkan agar script otomatis menggunakan jalur ANONSURF (TOR)
PROXY_CONFIG = ""  

# User Agents agar terlihat seperti device asli
USER_AGENTS = [
...
def save_result(service, target, password, note=""):
    ...
    # SOC Analysis Engine: Menilai kekuatan password yang ditemukan
    weakness = []
...
def print_banner():
    ...
    try:
        # Menggunakan font standar agar lebih rapi
    ...
    print(colored("+" + "-" * 68 + "+", 'white'))
    print(colored("\n[!] INFO: Pastikan 'Anonsurf' aktif sebelum menjalankan script ini.", 'red'))
...
async def ig_engine(passwords):
    ...
    print(colored("[!] Mode: Stealth (Delay 10-12s) agar tidak diblokir.", 'cyan'))
    ...
    async with ClientSession() as session:
        for pwd in passwords:
            ...
            # --- DELAY LOGIC (Sesuai Request: ~10 Detik) ---
            # Menggunakan random antara 10 sampai 12 detik agar terlihat manusiawi
            delay = random.randint(10, 12)
            sys.stdout.write(f"[Sleep {delay}s]")
            sys.stdout.flush()
...
def main():
    while True:
        ...
        print("3. Instagram (Async - Stealth 10s Delay)")
